...
/// <summary>
/// Set a uGUI indicator by positioning and rotating it, so it always indicate the gameObject when it's offscreen (and eventually onScreen)
/// </summary>
...

{
    ...
    public class Pointer : MonoBehaviour
    {
        ...
        /// </summary>
        [HideInInspector()]
        public const float version = 0.7f;
        /// <summary>
        /// See "mode".
        /// </summary>
        public enum mode_enum { SCREEN = 1, DISC = 2 };
        /// <summary>
        /// The mode chosen to drive the way the UI Image are displayed
        ///     - SCREEN : The UI Images are bounded on the borders of the screen.
        ///     - DISC : The UI Images are bounded on a circle placed at the center of the screen.
        /// </summary>
        public mode_enum mode = mode_enum.SCREEN;
        /// <summary>
        /// Indicate if the radius is automatically computed (mode needs to be set to mode_enum.CIRCLE)
        /// </summary>
        public bool autoSetRadius = true;
        /// <summary>
        /// Radius of the circle from the middle of the screen in pixels (mode needs to be set to mode_enum.CIRCLE and autoSetRadius to false);
        /// </summary>
        public float radius = 0f;

        /// <summary>
        /// Display sur Gizmo helper to evaluate the onScreen surface. (Editor only)
        /// </summary>
        public bool displayGizmo = false;

        /// <summary>
        /// The pointer to display when the gameObject is offscreen.
        /// </summary>
        public Image pointer;

        /// <summary>
        /// The target (optional) to display when the gameObject is on screen.
        /// Set it to null if you don't want to use it.
        /// </summary>
        public Image target;

        /// <summary>
        /// The camera to use with the tracking. If null, the main Camera is used by default.
        /// </summary>
        public Camera cam;

        /// <summary>
        /// Sometimes, the UI Image is not correctly oriented, use this offset to rotate the pointer.
        /// </summary>
        public float offsetRotation = 0f;

        /// <summary>
        /// Use the padding to offset pointer, from the chosen bounds (CIRCLE or SCREEN)
        /// </summary>
        public Vector2 padding = new Vector2(90f, 90f);


        /// <summary>
        /// The event triggered when the target enter the screen area.
        /// (see usage in documentation)
        /// </summary>
        [SerializeField]
        public UnityEvent OnEnterScreen;

        /// <summary>
        /// The event triggered when the target exit the screen area.
        /// (see usage in documentation)
        /// </summary>
        [SerializeField]
            ...
            try
            {
                ...
                switch (canvas.GetComponent<CanvasScaler>().uiScaleMode)
                {
                    ...
                    case CanvasScaler.ScaleMode.ConstantPhysicalSize:
                        Debug.LogError("OffScreen Indicator doesn't support Constant physical size canvas scaler mode.");
                        return;
                        break;
                    case CanvasScaler.ScaleMode.ScaleWithScreenSize:
                        Debug.LogError("OffScreen Indicator doesn't support Scale with screen size canvas scaler mode.");
                        return;
                        break;
                }
        ...
        /// <summary>
        /// Compute the pointer position when the gameObject is offscreen. (SCREEN mode only)
        /// </summary>
        /// <param name="behind">Indicate if the target is behind the camera</param>
        /// <returns>A Vector2 coordinates of the pointer position on screen.</returns>
